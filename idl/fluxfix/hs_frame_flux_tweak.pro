...
;
; CALLING SEQUENCE:
;   frame_flux_tweak, loglam, flux, ivar $
;     best_exp, plugtag, corrfile, diag=
;
; INPUTS:
;   loglam  -  wavelength array in (log10 Ang.) [nred_pix, nfiber*nexp] 
;   flux    -  flux array (uncalibrated) [nred_pix, nfiber*nexp]
;   ivar    -  inverse variance [nred_pix, nfiber*nexp]
...
;                low S/N cases
;   April 2004   Updated by R. Cool to allow for reduction of hectospec
;-
;------------------------------------------------------------------------------

pro hs_frame_flux_tweak, loglam,flux, ivar, best_exp, plugtag, $
                         corrfile, diag = diag
   ...
   bfit = spmedian_rebin(loglam[*,indx], flux[*,indx], ivar[*,indx], $
          'full', outwave = bwave, mask = bmask, sn = bsn, quality = bquality)
   ...
   medloglam = [bwave] # replicate(1,nfiber) 
   ...
   for ifile=0, nfiles-1 do begin
      ...
      bscifit = spmedian_rebin(loglam[*,indx], flux[*,indx], ivar[*,indx], $
                'full', mask = bscimask, sn = bscisn, quality = bsciquality)
      ...
      poly3 = where(scisnmed GT 3.0  $
               AND   bestsnmed GT 3.0 $
               , npoly3) 
      if poly3[0] NE -1 then fiber_coeff[poly3] = 3 

      poly2 = where(scisnmed GT 1.0  $
               AND  bestsnmed GT 1.0 $
      ...
      if npoly1 gt 0 and (npoly2 gt 0 or npoly3 gt 0) then begin
        ...
        for ifib = 0, npoly1 - 1 do begin
          ...
          nearindx = sort(dist)
          near5 = hisn[nearindx[0:(4<(n_elements(nearindx)-1))]]
     ...
     ;------------
      ...
      if keyword_set(diag) then begin
        ...
        for ii = 0, nstd - 1 do begin
          istd = std[ii]
          plot, wave, corrimage[*,istd], yr=[0., 2], /nodata, $
      ...
      djs_plot, wave, corrimage, /nodata, yr=[0, 2], $
