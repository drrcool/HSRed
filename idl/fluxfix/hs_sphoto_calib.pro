...
pro hs_sphoto_calib, wave, flux, invvar, mask, plugtag, fcalfile, stdstarfile, $
  stype = stype, input_calibset = input_calibset, noplot = noplot, superfile=superfile
  ...
  frames = plugtag[uniq(plugtag.frames, sort(plugtag.frames))].frames
  ...
  if wave(0,0) gt wave(1,0) then begin
     wave = reverse(wave)
     flux = reverse(flux)
     invvar= reverse(invvar)
     mask = reverse(mask)
  endif
  ...
  ; If the superfile keyword is set, then read that file, and find the standards
  ; there. If there are missing stars, use those
  ;----------------------------------------------------------------------------
 
 
 typeon = 0

if keyword_set(stype) and $
   not keyword_set(superfile) then begin
    ...
    stdinfo = hs_stype_standard(newwave, avgflux, avginvar, plug1, $
              outfile = stdstarfile)

    
   endif 
 
  if keyword_set(superfile) then begin
  
    
    supertype = mrdfits(superfile,1)
   ...
   spherematch, ra1, dec1, ra2, dec2, 1.0, match1, match2, dist
   
   if match1(0) ne -1 and match2(0) ne -1 then begin
      stdinfo2 = supertype[match2]
      stdinfo2.fiberid = plug1[match1].fiberid
   
      
   endif
   
   
   if n_elements(match1) ne n_elements(ra1) then begin
      typeon = 1
      splog, 'Not all of the standards were found in the superfile'
      splot, 'Typing the stars now'
   endif
   
   mwrfits, stdinfo2, stdstarfile, /create
  ...
  ok = where(stdinfo_all.sn gt 7 and abs(stdinfo_all.v_off) lt 450 and $
  ...
  if nok lt 3 then begin
    ...
    splog, 'Proceeding anyway!'
    ok = where(stdinfo_all.sn gt 7 and stdinfo_all.mag[2] gt 0, nok)
    nok = nok / nframes
  endif else begin
  ...
  pca_calibset = hs_pca_flux_standard(newwave, newflux[*,ok], newivar[*,ok], $
  ...
  if keyword_set(input_calibset) then begin
     ...
     splog, 'Using the input calib set instead of the PCA average'
  ...
  for iframe = 0, nframes - 1 do begin
    indx = where(plugtag[ok].frames eq frames[iframe], nindx)
   ...
   ; endif
    ...
    frame_calibset = hs_frame_flux_calib(newwave, corvector[*,indx], $
      corvivar[*,indx], avgcalibset, cormed[indx], framename = $
      'Frame -' + string(frames[iframe], format='(i3.3)'), fit_wiggles = (medsn gt 100), $
