...
pro kurucz_match, wave, nflux, nivar, nkflux, kindx, fiber, $
   ...
   for imod = 0, nummod - 1 do begin
    ;  print,  where(wave gt 3850 and wave lt 6000 and nivar ne 0 )
    ;  print, where(finite(nflux) and finite(nivar)) 
     goodpix = where(wave gt 3850 and wave lt 6000 and nivar ne 0 $
                     and finite(nflux) and finite(nivar), ct) 
...
function hs_stype_standard, loglam, nflux, ninvvar, plugmap, outfile = outfile, $
   ...
   hs_kurucz_restore, kwave, kflux, nkflux = nkflux, kindx = kindx, $
                   smoothpix = smoothpix
   nk = n_elements(kindx)
   ...
   zans = hs_zcompute(nflux, ninvvar, nkfluxi, starmask, $
