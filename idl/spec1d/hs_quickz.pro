...
;------------------------------------------------------------------------------
FUNCTION hs_quickz, platefile, doplot=doplot, debug=debug, fiberid=fiberid, $
  ...
  if NOT keyword_set(wavemax) then wavemax=8500
   ...
   npoly = 5
   IF NOT keyword_Set(zmin) THEN zmin = -0.01                 ; -3000 km/sec
   IF NOT keyword_Set(zmax) THEN zmax = 1.0 ; 
   pspace = 2
   nfind = 5
...
;   for iper=nper-1, 0, -1 do begin
   ...
   if (ww[1<(n_elements(ww)-1)] EQ 'fnal') then return, -1
