...
;------------------------------------------------------------------------------
function hs_zcompute, objflux, objivar, starflux, starmask, nfind=nfind, $
 ...
 plottitle=plottitle, doplot=doplot1, debug=debug, verbose=verbose
   ...
   if (nobj GT 1) then begin
      ...
      for iobj=0L, nobj-1 do begin
         ...
         zans1 = hs_zcompute(objflux[*,iobj], objivar[*,iobj], $
...
;      zans[0:npeak-1].chi2 = -ypeak1
      xpeak1 = hs_find_nminima(chi2arr[indx], lags[indx], $
