...
;------------------------------------------------------------------------------
pro hs_reduce1d, platefile, doplot=doplot, debug=debug, fiberid=fiberid, $
...
;  if NOT keyword_set(wavemax) then wavemax=8500
  if NOT keyword_set(wavemin) then wavemin=3800
  if NOT keyword_set(wavemax) then wavemax=8900
...
; edited 02/10/14 by S Moran to skip telluric correction, as it can
; (should) now be done earlier in the code
