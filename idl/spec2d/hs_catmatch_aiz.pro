PRO hs_catmatch_aiz, mapfile, catalog=catalog,outname=outname
  ...
  k = where(strmatch(target1, 'sky*') eq 0 and strmatch(target1, 'unused*') eq 0)
  fiber1 = fiber1(k)
  target1=target1(k)
  ra1=ra1(k)
  dec1=dec1(k)
  ...
  if file_test(catalog+'_sky') eq 1l then begin
  ;;Now match the skies
