...
function arcfit_iter, spec, loglam, intensity, $
 aset, dcoeff, nsteps, nsmooth=nsmooth, dlag=dlag, bestcorr=bestcorr, model=model
...
;      istep, nsteptot, string(13b)
   ...
   xy2traceset, dindgen(npix)-lagbest, bestlambda, wset, ncoeff=ncoeff, $
    xmin=0, xmax=npix-1, maxiter=1, /silent
...
function iarcfit_guess, spec, loglam, intensity, color=color, func=func, $
 bestcorr=bestcorr, acoeff=acoeff, dcoeff=dcoeff, nsteps=nsteps, model=model
...
;  splog, 'Searching about coefficients = ', aset.coeff, format='(a,99f12.5)'
   if not keyword_set(nsteps) then nsteps = [1, 20, 5, 1]
   wset = arcfit_iter(spec, loglam, intensity, $
    aset, dcoeff, nsteps, nsmooth=6.0, dlag=2, bestcorr=bestcorr,model=model)

   ; Second search varies 4 coefficients, but narrows search window
   ; for the first two coefficients
;  splog, 'Searching about coefficients = ', wset.coeff, format='(a,99f12.5)'
   nsteps = [1, 5, 5, 5]
   dcoeff[0] = dcoeff[0] / 5. ; Narrow search on coefficient #0
   dcoeff[1] = dcoeff[1] / 6. ; Narrow search on coefficient #1
   wset = arcfit_iter(spec, loglam, intensity, $
    wset, dcoeff, nsteps, nsmooth=4.0, dlag=1, bestcorr=bestcorr,model=model)

;  splog, 'Initial wavelength fit = ', wset.coeff, format='(a,99f12.5)'
   splog, 'Initial cross-correlation coefficient = ', bestcorr
