...
;-
;------------------------------------------------------------------------------
function trace150crude, image, invvar, ystart=ystart, nmed=nmed, $
                        xmask=xmask, radius=radius, yset=yset, $
                        maxerr=maxerr, maxshifte=maxshifte, $
                        maxshift0=maxshift0, xerr=xerr, maxdev=maxdev, $
                        ngrow=ngrow, chelle=chelle,$
                        fibermask=fibermask, nfiber=nfiber, $
                        npbundle=npbulde, deltax=deltax
   
   IF NOT keyword_set(nfiber) THEN nfiber = 150.0
   ...
   ;----------
   ; Find the 150 X-centers in the row specified by YSTART

   ; XGOOD=1 for fibers that were actually found, 0 otherwise
   xstart = trace150cen(fimage, mthresh=mthresh, ystart=ystart, nmed=nmed, $
                        xgood=xgood, nfiber=nfiber, $
                        chelle=chelle, deltax=deltax, $
                        npbundle=npbundle)
   ntrace = N_elements(xstart) ; Better be 150
   ...
   ; by TRACE150CEN.  Those fibers might not actually be bad, but might
   ...
   ;  ) Calculate the offset of the problem trace with the selected neighbors
