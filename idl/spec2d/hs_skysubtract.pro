...
;-
;------------------------------------------------------------------------------
function hs_skysubtract, objflux, objivar, plugsort, $
                         wset, objsub, objsubivar, $
                         iskies=iskies, fibermask=fibermask, $
                         nord=nord, upper=upper, $
                         lower=lower, maxiter=maxiter, $
                         pixelmask=pixelmask, thresh=thresh, $
                         dispset=dispset, npoly=npoly, $
                         relchi2struct=relchi2struct, $
                         novariance=novariance, tai=tai, jd=jd, nbkpt=nbkpt, $
  ...
  iskies = where(strmatch(plugsort.objtype,  'SKY*') $
  ...
  if NOT keyword_set(tai) and NOT keyword_set(jd) then airmass = replicate(1.0, nrow) $
  else airmass = float(tai2airmass(plugsort.ra, plugsort.dec, tai=tai, $
                       jd=jd, altitude=2606.,longitude=(360.-(110.+53./60.+4.4/3600.)),latitude=(31.+41/60.+19.6/3600.)))
  ...
  sset = bspline_iterfit(skywave, skyflux, invvar=skyivar, $
                         nord=nord, upper=upper, lower=lower, $
                         maxiter=maxiter, $
                         /eachgroup, everyn=everyn, bkpt=bkpt, $
  ...
  if (npos GT n_elements(gkern) AND nbkpt GE 16) then begin
     ...
     sset = bspline_iterfit(skywave, skyflux, invvar=skyivar, $
                            nord=nord, upper=upper*1.5, $
                            lower=lower*1.5, maxiter=maxiter, $
                            /eachgroup, fullbkpt=fullbkpt, $
                            yfit=skyfit, outmask=outmask)
  ...
  if (keyword_set(dispset)) then begin
     ...
     sigma = sigma - 1.0
     for v=0, n_elements(sigma[0,*])-1 do sigma[*,v]=dispset.median[v]
     skysigma = (sigma[*,iskies])[isort]
     
     fullx2 = replicate(1.0,ncol) # findgen(nrow)
     ;pad the ends with the row number of the first/last sky to prevent extrapolation
     fullx2[*,0:min(iskies)]=min(iskies)
     fullx2[*,max(iskies):*]=max(iskies)
     x2 = (fullx2[*,iskies])[isort]
     fiber_radius=sqrt(plugsort.xfocal^2.0+plugsort.yfocal^2.0)
     ;x2=replicate(1.0,ncol) # fiber_radius[iskies]

     ;make sure there are at least nord+npoly+1 points between each
     ;breakpoint, or else do the simple everyn=nskies version
     bkpt_diff=fullbkpt-shift(fullbkpt,1)
     min_diff=min(bkpt_diff[1:*])
     dlam=median(wave[*,iskies]-shift(wave[*,iskies],1))
     if min_diff/dlam*nskies gt nord+npoly+1 then begin  
        sset2d = bspline_iterfit(skywave, skyflux, invvar=skyivar*outmask, $
                          nord=nord, upper=upper, lower=lower, $
                          maxiter=maxiter, funcname='legendre', $
                          npoly = npoly, fullbkpt=fullbkpt, $ ;everyn=nskies+2, $ ; 
                          yfit=skyfit, x2=x2) ; , xmin=0., xmax=nrow) ;x2=skysigma
     endif else begin
        sset2d = bspline_iterfit(skywave, skyflux, invvar=skyivar*outmask, $
                          nord=nord, upper=upper, lower=lower, $
                          maxiter=maxiter, funcname='legendre', $
                          npoly = npoly, everyn=nskies, $ ; 
                          yfit=skyfit, x2=x2) ; , xmin=0., xmax=nrow) ;x2=skysigma
     endelse
     if (keyword_set(sset2d)) then begin
        sset = sset2d
        fullfit = bspline_valu(wave, sset, x2=fullx2) ;sigma) 
     endif
   ...
   if (posvar[0] NE -1) then begin
      ...
      skyvarset = bspline_iterfit(skywave[posvar], skyvariance, $
                                  invvar=skyivar[posvar], nord=nord, $
   ...
   if (nskies GE 3 AND ngoodpix GT ncol $
      ...
      splog, 'WARNING: Too few sky fibers or pixels to model ' + $
         ' sky-sub variance!!'
      relchi2fit = 1
   ...
   if (keyword_set(relchi2) AND keyword_set(newmask)) then begin
      ...
      hs_redmonster, relwave, relchi2, wave, thresh=thresh, pixelmask=newmask
   endif
   ...
   if (keyword_set(newmask)) then begin
      ...
      qbright = (objsubivar NE 0) $
         AND (fullfit GT objsub + 10.0 / sqrt(objsubivar + $
                                              (objsubivar EQ 0))) $
