...
pro fitarcimage, arc, arcivar, xcen, ycen, wset, wfirst=wfirst, $
  ...
  xcen = trace_crude(arc, yset=ycen, nave=1, nmed=1, xstart=xstart, $
   ...
   if (keyword_set(gauss)) then begin
      x2  = trace_gweight(arc, xmid, ycen, sigma=1.0, $
                          invvar=arcivar, xerr=xerr) 
      x2  = trace_gweight(arc, x2, ycen, sigma=1.0, $
                          invvar=arcivar, xerr=xerr) 
      xcen = trace_gweight(arc, x2, ycen, sigma=1.0, $
                           invvar=arcivar, xerr=xerr) 
   endif else begin  
      x2  = trace_fweight(arc, xmid, ycen, radius=2.0, $
                          invvar=arcivar, xerr=xerr)
      x2  = trace_fweight(arc, x2, ycen, radius=2.0, $
                          invvar=arcivar, xerr=xerr)
      xcen = trace_fweight(arc, x2, ycen, radius=2.0, $
                           invvar=arcivar, xerr=xerr)
   endelse
  ...
  ;;---------------------------------------------------------------------------
   ...
   if (nlamp LT 6) then begin
      splog, 'WARNING: Reject arc. Number of arclines after bundle test = ', $
         nlamp
      wset = 0
      return
   endif else begin
   ...
   if bind[0] NE -1 then begin
      ...
      for jj=0,n_elements(bind)-1 do $
         splog, bind[jj], lamps[bind[jj]].lambda, $
         (total(testg EQ 0,2))[bind[jj],*], $
   ...
   for i=0, N_elements(logwdiff)-1 do begin
      if (logwdiff[i] GT 0.10) then begin
         ...
         splog, 'WARNING: Reject arc. Big wavelength gap from ', $
            10^logwlist[i], $
