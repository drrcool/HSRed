...
;-
;------------------------------------------------------------------------------
pro hs_extract_image, fimage, invvar, xcen, sigma, flux, finv, yrow=yrow, $
...
;
   ...
   for iy=0, nRowExtract-1 do begin
     ...
     pixelmasktemp = 0
     ansrow = hs_extract_row(fimage[*,cur], invvar[*,cur], $
     ...
     calcflux, ansrow, prow, fluxrow, finvrow, wfixed, proftype,$
        lTrace,nCoeff,$
     ...
     if(ARG_PRESENT(pixelmask)) then begin
       ...
       if (whoppingct GT 0) then begin
          ...
          if (squashprofile) then wflux = $
             ansrow[nTrace+nPoly:nTrace+nPoly+whoppingct-1]$
         else wflux = ansrow[ma-whoppingct:ma-1]

         for ww = 0,whoppingct - 1 do begin
           guessdist = abs(xcen[cur,whopping[ww]] - xcen[cur,*])/wsigma
           guessflux = exp(-guessdist) * (wflux[ww]/wsigma) *$
              (guessdist LT 5.0)
           crosstalk = (guessflux GT 0.5 * abs(fluxrow))
           crosstalk[ww] = 0
           pixelmask[cur,*] = pixelmask[cur,*] OR $
              (pixelmask_bits('CROSSTALK') * crosstalk)
         endfor
       endif
