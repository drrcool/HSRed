...
;-
;------------------------------------------------------------------------------
function trace150cen, fimage, mthresh=mthresh, ystart=ystart, nmed=nmed, $
                      xgood=xgood, nfiber=nfiber, chelle=chelle, $
                      npbundle=npbundle, deltax=deltax
   
   IF NOT keyword_set(nfiber) THEN nfiber = 150

   ; Need 1 parameter
   if (N_params() LT 1) then begin
      print, $
         'Syntax - xfiber = trace150cen( fimage, [ mthresh=, ystart=, nmed= ]'
      return, -1
   endif

   nx = (size(fimage))[1]
   ny = (size(fimage))[2]
   if (NOT keyword_set(ystart)) then ystart = 2000
   if (NOT keyword_set(nmed)) then nmed = 8

   nfiber = nfiber     ; Number of fibers per side
   
   IF keyword_set(chelle) THEN npbundle = 1
;   nbun = nfiber / npbundle     ; Number of bundles
   bin = 2048/n_elements(fimage(*,0))
   IF NOT keyword_set(deltax) AND  keyword_set(chelle) THEN deltax = 12.0/bin
   IF NOT keyword_set(npbundle) THEN npbundle = 1
   IF NOT keyword_set(deltax) THEN deltax = 8.0
   nbun = nfiber / npbundle
   ...
   ; Go through a first iteration of finding 150 centers, but just to
