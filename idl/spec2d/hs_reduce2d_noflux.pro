...
;;This program will complete the 2-D reductions for a directory.  You should have already run 
;;hs_preproc and examined the lists.

PRO hs_reduce2d_noflux, path=path, rerun=rerun, outdir=outdir,$
  ...
  if direxist(path+'calibration') eq 0L then spawn, 'mkdir ' $
     + path+'calibration'
  if direxist(path+'calibration/' + rerun) EQ 0L then spawn,$
     'mkdir ' + path+'calibration/' + rerun
  ...
  if direxist(path + 'reduction') EQ 0 then spawn, 'mkdir ' + $
     path + 'reduction'
 ...
 readcol, listpath + 'cal.list', filelist, plugmap1, $
    format='A,A', comment='#', $
    delimiter=' '
 ...
 out = 'spHect-' + string(pass(match),format='(i1.1)') + $
    string(field(match),format='(i2.2)') + $
       '-' + rerun + '.fits'
 hs_extract, filelist, plugfile=plugmap, /tweaksky, $
    /fiberflat, /skysubtract, /writeout, /writeall,$
    /combine, /plugcat, outdir=redpath, outname=out , $
