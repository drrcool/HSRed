...
;-
;------------------------------------------------------------------------------
function hs_telluric_corr,flux, fluxivar, wset, plugsort, $
 ...
 plottitle=plottitle, write=write
...
;       cwave2: [8644., 8737., 0] }
   ...
   for iband=0, n_elements(tellbands)-1 do begin
      ...
      for i=0, n_elements(tellbands[iband].cwave1)-1 do $
       cmask = cmask $
        OR (loglam GE (tellbands[iband].cwave1[i]) $
        AND loglam LE (tellbands[iband].cwave2[i]))
      for i=0, n_elements(tellbands[iband].twave1)-1 do $
       tmask = tmask $
        OR (loglam GE (tellbands[iband].twave1[i]) $
        AND loglam LE (tellbands[iband].twave2[i]))
   ...
   if keyword_set(write) then mwrfits, tellset, 'reduction/tell.fits'
   ...
   if (keyword_set(plottitle)) then begin
      ...
      for iband=0, n_elements(tellbands)-1 do begin
         ...
         if (indx[0] NE -1) then begin
            ...
            djs_oplot, fitloglam[indx], tellplot
         endif
