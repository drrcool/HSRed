...
;------------------------------------------------------------------------------
pro hs_combine1fiber, inloglam, objflux, objivar, $
 finalmask=finalmask, indisp=indisp, skyflux=skyflux, $
 newlam=newlam, newflux=newflux, newivar=newivar, $
 ...
 _EXTRA=KeywordsForReject, verbose=verbose
   ...
   nfinalpix = n_elements(newlam)
   
  
   
   
   if (npix EQ 0 OR nfinalpix EQ 0 OR n_params() LT 2) then $
    message, 'INLOGLAM, OBJFLUX and NEWLAM are all required'
   ...
   if (NOT keyword_set(binsz)) then begin
      ...
      else binsz = (newlam[nfinalpix-1] - newlam[0])/nfinalpix
   ...
   if (ngood EQ 0) then begin
      ...
      for igrp=0L, nstart-1 do begin
         ...
         inside = where(newlam GE min(inloglam[ss])-EPS $
          AND newlam LE max(inloglam[ss])+EPS, numinside)
...
; Another work-around for the Slatec code ???
         if (numinside LE 2) then begin
            ...
            newflux[inside] = bspline_valu(newlam[inside], sset, $
                                   mask=bvalumask)
...
;                bspline_valu(inloglam[ss[ireplace]], sset)
      ...
      for j=0L, max(specnum) do begin
         ...
         if (these[0] NE -1) then begin
            ...
            inbetween = where(newlam GE min(inloglam[these]) AND $
                              newlam LE max(inloglam[these]))
            if (inbetween[0] NE -1) then begin
               ...
               if (arg_present(newivar)) then begin
                  ...
                  result = interpol(objivar[these] * fullcombmask[these], $
                   inloglam[these], newlam[inbetween]) 
               ...
               lowside = fix((inloglam[these]-newlam[0])/binsz)
               highside = lowside + 1
   ...
   if (keyword_set(newivar)) then begin
      ...
      ; Interpolate over masked pixels, just for aesthetic purposes
       ...
       if goodpts[0] NE -1 then begin
         minglam = min(newlam[goodpts])
         maxglam = max(newlam[goodpts])
         
         ibad = where(newlam LT minglam OR newlam GT maxglam)
