...
;-
;------------------------------------------------------------------------------
pro hs_lacosmic, imlist, outlist=outlist, masklist=masklist, sigclip=sigclip, $
                 gain=gain, readn=readn, skyval=skyval,$
                 objlim=objlim, niter=niter,$
                 sigfrac=sigfrac, $
                 verbose=verbose,statsec=statsec,zeroindexed=zeroindexed,$
                 masksuffix=masksuff, outsuff=outsuff, isbig=isbig,$
                 blocksize=blocksize
...
;; set some sensible defaults
    ...
    if ( (not keyword_set(masklist)) and (not keyword_set(masksuff)))$
       then begin
       masksuff = "-mask"
    endif
; jm01dec18uofa
; ----------------------------------------
    ...
    if size(imlist,/type) eq 7L then nfiles = n_elements(imlist) else begin
       ...
       case ndim of
          1L: begin
             if size(imlist,/type) eq 7L then nfiles = n_elements(imlist) $
             else $
...
; read the string array of FITS files or the data cube [jm01dec18uofa]
       ...
       if size(imlist,/type) eq 7L then begin
          ...
          if (verbose) then print, "LACOSMIC: Working on image "$
             + string(i,format='(I0)')
          oldoutput = imlist[*,*,i]
       ...
       while(not sstop) do begin
          ...
          if (skyval[i] gt 0.0) then oldoutput = temporary(oldoutput) $
             + skyval[i]
          ...
          for xchop = 0, potx - 1 do begin
             for ychop = 0, poty - 1 do begin
                ...
                im2 = [0]
                sigmap =  -1.0 * $
                   (djs_median(sigmap,width=5,boundary='reflect') - $
                  temporary(sigmap))
                ...
                inputmask = $
                   lacos_replace(inputmask, !VALUES.F_NAN,'INDEF',-9999)
                                ;med5 = median(inputmask,5,/even)*outmask
                med5 = $
                   djs_median(inputmask,width=5,boundary='reflect')*outmaskwork
                inputmask = [0]
                output = (1.0 - outmaskwork)*oldoutputwork + med5
                med5 = [0]
                oldoutput[regx[xchop,0]:regx[xchop,1],$
                          regy[ychop,0]:regy[ychop,1]] = $
                  output[0:(tmpxsize-1),0:(tmpysize-1)]
                outmask[regx[xchop,0]:regx[xchop,1],$
                        regy[ychop,0]:regy[ychop,1]] = $
          ...
          if verbose then print, 'Found ' + $
             strtrim(string(ncosmicray)) + $
             ' cosmic rays in iteration ' + strtrim(string(iter))
