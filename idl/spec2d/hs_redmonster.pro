...
;-
;------------------------------------------------------------------------------
pro hs_redmonster, relloglam, relchi2, objloglam, filtsz=filtsz, $
 thresh=thresh, pixelmask=pixelmask
   ...
   badmask = convol(long(badmask), lonarr(filtsz)+1L, $
   ...
   while (total(badmask) NE 0) do begin
      ...
      splog, string(fix(relloglam[i0]), fix(relloglam[i1]), peakchi, $
      format='("WARNING: Bad sky residuals at ", i5, " to ", i5, " Ang; Peak chi=", f6.2)')
      if ( relloglam[i0] GT 6250 AND relloglam[i0] LT 6500 $
       AND relloglam[i1] GT 6500 AND relloglam[i1] LT 6750) then begin
         splog, string(fix(relloglam[i0]), fix(relloglam[i1]), peakchi, $
      ...
      if (keyword_set(objloglam) AND keyword_set(pixelmask)) then begin
         indx = where(objloglam GE relloglam[i0] AND objloglam $
                      LE relloglam[i1])
         if (indx[0] NE -1) then $
