...
;        sigma= 2.0
        ...
        if keyword_set(qaplot) or keyword_set(doplot)  then begin
         ...
         if keyword_set(skysubtract) and nother gt 3 and nccd gt 3 $
            ...
            even=indgen(75)*2 ;even/odd in idl indexing, not fiber num
            odd=indgen(75)*2+1
            traceset2xy, airset, xx,lam
            xy2traceset, xx[*,even], lam[*,even], airset_e, ncoeff=6
            xy2traceset, xx[*,odd], lam[*,odd], airset_o, ncoeff=6
            ...
            xy2traceset, xx, disp[*,k], tempdisp, ncoeff=3
...
;            skystuct_e = hs_skysubtract(specflux[*,even], $
;                                      specfluxivar[*,even], plugmap[even], $
;                                      airset_e,   objsub1_e, objsubivar1_e, $
;                                    newmask=newmask1_e, sset=sset_e, jd=jd) ;,$
;                                      ;dispset=dispset)
;            skystuct_o = hs_skysubtract(specflux[*,odd], $
;                                      specfluxivar[*,odd], plugmap[odd], $
;                                      airset_o,   objsub1_o, objsubivar1_o, $
;                                    newmask=newmask1_o, sset=sset_o, jd=jd) ;,$
;                                      ;dispset=dispset)
;            sset=[sset_e,sset_o]
;            objsub1=[[objsub1_e],[objsub1_o]]
;            objsub1[*,even]=objsub1_e
;            objsub1[*,odd]=objsub1_o
;            objsubivar1=[[objsubivar1_e],[objsubivar1_o]]
;            objsubivar1[*,even]=objsubivar1_e
;            objsubivar1[*,odd]=objsubivar1_o
;            newmask1=[[newmask1_e],[newmask1_o]]
;            newmask1[*,even]=newmask1_e
;            newmask1[*,odd]=newmask1_o

            skystuct = hs_skysubtract(specflux, $
                                      specfluxivar, plugmap, $
                                      airset,   objsub1, objsubivar1, $
                                    newmask=newmask1, sset=sset, jd=jd) ;,$
                                      ;dispset=dispset)
...
evenodd=0
if evenodd ne 1 then begin
            ;second pass sky subtraction
            k = where(strmatch(plugmap.objtype, 'TARGET*'))
            skies=where(strmatch(plugmap.objtype, 'SKY*'))
            for mm=0, n_elements(k)-1 do begin
                distance=distarcsec(plugmap[k[mm]].ra, plugmap[k[mm]].dec, plugmap[skies].ra, plugmap[skies].dec)
                closesttwo=(distance[sort(distance)])[0:1]
                closesttwo=skies[where((distance eq closesttwo[0]) or (distance eq closesttwo[1]))]
                 delvar, xx
                traceset2xy, airset, xx,lam_2nd
                wave_ref=lam_2nd[*,k[mm]]
                skyfit1=interpol(objsub[*,closesttwo[0]],lam_2nd[*closesttwo[0],wave_ref, /spline)
                skyfit2=interpol(objsub[*,closesttwo[1]],lam_2nd[*closesttwo[1],wave_ref, /spline)
;                sset = bspline_iterfit(lam_2nd[*,closesttwo], objsub[*,closesttwo], $
;                                    invvar=objsubivar[*,closesttwo], $
;                                    yfit=skyfit, outmask=outmaska, upper=2, lower=2, $
;                                    bkpt=bkpt, nbkpt=5, /eachgroup)
;                  skyfit1=bspline_valu(lam_2nd[*,closesttwo[0]], sset)
;                  skyfit2=bspline_valu(lam_2nd[*,closesttwo[1]], sset)
                  djs_iterstat, objsub[*,closesttwo[0]]-skyfit1, mask=outmask1, sigrej=4
                  djs_iterstat, objsub[*,closesttwo[1]]-skyfit2, mask=outmask2, sigrej=4
                  ;don't want to use bad point in the sky fiber as part of our template for subtracting residuals
                  inputbad=where((objsubivar[*,closesttwo[0]] eq 0.0),badcnt)
                  if badcnt ge 1 then outmask1[inputbad]=1 
                  inputbad=where((objsubivar[*,closesttwo[1]] eq 0.0), badcnt)
                  if badcnt ge 1 then outmask2[inputbad]=1 
                  ;lets only subtract residuals where both skies show high resids
                  wave_sort=[lam_2nd[*,closesttwo[0]],lam_2nd[*,closesttwo[1]]]
                  order=sort(wave_sort)
                  wave_sort=wave_sort[order]
                  mask_sort=[outmask1, outmask2]
                  mask_sort=mask_sort[order]
                  outmask=mask_sort
                  flux_sort=[objsub[*,closesttwo[0]],objsub[*,closesttwo[1]]]
                  flux_sort=flux_sort[order]
                  ivar_sort=[objsubivar[*,closesttwo[0]],objsubivar[*,closesttwo[1]]]
                  ivar_sort=ivar_sort[order]
                  ;grow the mask by +-3 pixels
                  highpeaks=[where(outmask eq 0)-5,where(outmask eq 0)-4,where(outmask eq 0)-3,where(outmask eq 0)-2,where(outmask eq 0)-1, where(outmask eq 0), where(outmask eq 0)+1, where(outmask eq 0)+2,where(outmask eq 0)+3,where(outmask eq 0)+4,where(outmask eq 0)+5]
                  highpeaks=highpeaks[sort(highpeaks)]
                  highpeaks=highpeaks[uniq(highpeaks)]
                  highpeaks=highpeaks[where((highpeaks ge 0) and (highpeaks lt n_elements(skyfit)))]
                  new_subtract=skyfit*0.0
                  new_subtract[highpeaks]=flux_sort[highpeaks]-skyfit[highpeaks]
                  new_subtract=ivarsmooth2(new_subtract, ivar_sort, 3)
                  sset = bspline_iterfit(wave_sort, $
                                    new_subtract, yfit=skyfit2, maxiter=0, nbkpt=4000)
                  resid1=bspline_valu(lam_2nd[*,k[mm]], sset)
                  objsub[*,k[mm]]=objsub[*,k[mm]]-resid1
                   set_plot, 'x'
                  splot, wave_sort, new_subtract
                  soplot, wave_sort, skyfit2, color='red'
                  soplot, lam_2nd[*,k[mm]], objsub[*,k[mm]], color='blue'
                  soplot, lam_2nd[*,k[mm]], objsub[*,k[mm]]+resid1, color='green'
                  junk=''
                  read, junk
                  endfor

endif else begin
;second pass, even and odd
            k = where(strmatch(plugmap[even].objtype, 'SKY*'))
            fibernum=even
            distance_matrix=intarr(n_elements(k), n_elements(fibernum))
            for mm=0, n_elements(k)-1 do begin
                distance_matrix[mm,*]=abs(fibernum[k[mm]]-fibernum)
                other_skies=k[where(k ne k[mm])]
                distance_matrix[mm, other_skies]=9999
             endfor
            for mm=0, n_elements(k)-1 do begin                
                closest=where((distance_matrix[mm,*] eq min(distance_matrix, dimen=1)) and $
                              (distance_matrix[mm,*] ne 9999))
                delvarx, xx
                traceset2xy, airset_e, xx,lam_2nd

                if n_elements(closest) gt 1 then begin
                   sset = bspline_iterfit(lam_2nd[*,k[mm]], objsub[*,even[k[mm]]], $
                                    invvar=objsubivar[*,even[k[mm]]], $
                                    yfit=skyfit, outmask=outmask, upper=2, lower=2, $
                                    bkpt=bkpt, nbkpt=5, /eachgroup)
                  djs_iterstat, objsub[*,even[k[mm]]]-skyfit, mask=outmask, sigrej=4
                  ;don't want to use bad point in the sky fiber as part of our template for subtracting residuals
                  inputbad=where(objsubivar[*,even[k[mm]]] eq 0.0, badcnt)
                  if badcnt ge 1 then outmask[inputbad]=1 
                  ;grow the mask by +-3 pixels
                  highpeaks=[where(outmask eq 0)-3,where(outmask eq 0)-2,where(outmask eq 0)-1, where(outmask eq 0), where(outmask eq 0)+1, where(outmask eq 0)+2,where(outmask eq 0)+3]
                  highpeaks=highpeaks[sort(highpeaks)]
                  highpeaks=highpeaks[uniq(highpeaks)]
                  highpeaks=highpeaks[where((highpeaks ge 0) and (highpeaks lt n_elements(skyfit)))]
                  new_subtract=skyfit*0.0
                  new_subtract[highpeaks]=objsub[highpeaks,even[k[mm]]]-skyfit[highpeaks]
                  sset = bspline_iterfit(lam_2nd[*,k[mm]], $
                                    new_subtract, yfit=skyfit2, maxiter=0, nbkpt=4000)
                  for p=0, n_elements(closest)-1 do begin
                     resid1=bspline_valu(lam_2nd[*,closest[p]], sset)
                     objsub1[*,even[closest[p]]]=objsub[*,even[closest[p]]]-resid1
                  endfor
                  ;to make sure we don't redo a fiber that is equally close to another sky
                  distance_matrix[*,closest]=9999
                  distance_matrix[mm,closest]=0                    
               endif else begin
                  distance_matrix[*,closest]=9999
                  distance_matrix[mm,closest]=0                    
                  objsub1[*,even[closest]]=objsub[*,even[closest]]
                  objsubivar1[*,even[closest]]=objsubivar[*,even[closest]]
                  newmask1[*,even[closest]]=pixmask[*,even[closest]]
               endelse
            endfor
            objsub[*,even] = objsub1[*,even]

            k = where(strmatch(plugmap[odd].objtype, 'SKY*'))
            fibernum=odd
            distance_matrix=intarr(n_elements(k), n_elements(fibernum))
            for mm=0, n_elements(k)-1 do begin
                distance_matrix[mm,*]=abs(fibernum[k[mm]]-fibernum)
            ...
            for mm=0, n_elements(k)-1 do begin                
               ...
               ; stop
                delvarx, xx
                traceset2xy, airset_o, xx,lam_2nd

                if n_elements(closest) gt 1 then begin
                   sset = bspline_iterfit(lam_2nd[*,k[mm]], objsub[*,odd[k[mm]]], $
                                    invvar=objsubivar[*,odd[k[mm]]], $
                                    yfit=skyfit, outmask=outmask, upper=2, lower=2, $
                                    bkpt=bkpt, nbkpt=5, /eachgroup)
                  djs_iterstat, objsub[*,odd[k[mm]]]-skyfit, mask=outmask, sigrej=4
                  ;don't want to use bad points in the sky fiber as part of our template for subtracting residuals
                  inputbad=where(objsubivar[*,odd[k[mm]]] eq 0.0, badcnt)
                  if badcnt ge 1 then outmask[inputbad]=1 
                  ;grow the mask by +-3 pixels
                  highpeaks=[where(outmask eq 0)-3,where(outmask eq 0)-2,where(outmask eq 0)-1, where(outmask eq 0), where(outmask eq 0)+1, where(outmask eq 0)+2,where(outmask eq 0)+3]
                  highpeaks=highpeaks[sort(highpeaks)]
                  ...
                  highpeaks=highpeaks[where((highpeaks ge 0) and (highpeaks lt n_elements(skyfit)))]
                  new_subtract=skyfit*0.0
                  new_subtract[highpeaks]=objsub[highpeaks,odd[k[mm]]]-skyfit[highpeaks]
                  sset = bspline_iterfit(lam_2nd[*,k[mm]], $
                                    new_subtract, yfit=skyfit2, maxiter=0, nbkpt=4000)
                  for p=0, n_elements(closest)-1 do begin
                     resid1=bspline_valu(lam_2nd[*,closest[p]], sset)
                     objsub1[*,odd[closest[p]]]=objsub[*,odd[closest[p]]]-resid1
                  endfor
                  ;to make sure we don't redo a fiber that is equally close to another sky
                  distance_matrix[*,closest]=9999
                  distance_matrix[mm,closest]=0                    
                  ...
                  distance_matrix[*,closest]=9999
                  distance_matrix[mm,closest]=0                    
                  objsub1[*,odd[closest]]=objsub[*,odd[closest]]
                  objsubivar1[*,odd[closest]]=objsubivar[*,odd[closest]]
                  newmask1[*,odd[closest]]=pixmask[*,odd[closest]]
               endelse
            endfor
            objsub[*,odd] = objsub1[*,odd]
         endelse ;endelse for evenodd=1 or 0
