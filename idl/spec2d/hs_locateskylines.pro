...
;                R Cool U of A
;   April 2014 - Modified to find a shift + scale factor offset, if
;                enough lines.
;-
;------------------------------------------------------------------------------

pro hs_locateskylines, skylinefile, fimage, ivar, wset, xarc, $
                       arcshift=arcshift, $
  ...
  xpredict = transpose( traceset2pix(wset, (skywaves)) )
  ...
  junk = where(aveinvvar GT 1./(2.0)^2, ngood)
  
  if (ngood EQ 0) then begin
     splog, 'WARNING: No good sky lines (with std-dev less than 1.0 pixels)'
     skyshift = 0
     return
